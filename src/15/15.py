...
def find_zeta(x, fx, h):
    ...
    a = [0]
    temp = [1 for i in range(d.shape[0]-1)]
    a = a + temp
    b = [4 for i in range(d.shape[0])]
    c = a[::-1]
...
if __name__ == "__main__":
    ...
    with open("15dane.data", "r") as data:
        ...
        while(line):
            line = line.strip()
            parts = line.split(" ")
            x_l.append(float(parts[0]))
            f_l.append(float(parts[1]))
            line = data.readline()

    x, fx = np.array(x_l), np.array(f_l)
